...
void Mesh::ReScale_uniform(double lscale){
    setparameters();
...
void Mesh::BuildNeighborTable_nonmanifold(){
    ...
    for(int i=0;i<n_edges;++i){
        ...
        for(auto p_vf = vf_begin(p_ev[0]);p_vf != vf_end(p_ev[0]);++p_vf){
            auto p_fv = fv_begin(*p_vf);
            for(int k=0;k<3;++k){
                if(p_fv[k]==p_ev[1]){++efnum_nonmanifold[i];break;}
            }
        }
    ...
    //        if(vertices_non_manifold[i]){
    //            int unmmm = 0;
    //            for(auto p_ve = ve_begin(i);p_ve!=ve_end(i);++p_ve)if(edge_non_manifold[*p_ve])++unmmm;
    //            cout<<unmmm<<' ';
    //        }
    //    }
    //cout<<endl;
    //    for(int i=0;i<n_vertices;++i){
    //        auto vvnonnum = vvnon_num(i);
    //        auto p_vvnon = vvnon_begin(i);

    //        if(vertices_non_manifold[i]){
    //            for(int j=0;j<vvnonnum;++j){
    //                if(!vertices_non_manifold[p_vvnon[j]]){cout<<"nonononononno"<<endl;}
    //            }
    //        }else{
    //            if(vvnonnum!=vv_num(i))cout<<"gggggg"<<endl;
...
void Mesh::ComputeFaceNormal(bool isComputeVerticesNormal, vector<bool> *p_isinver){
    ...
    if(p_isinver!=NULL){
        for(int i=0;i<n_faces;++i)if(p_isinver->at(i))Utility::inversevec(fnor_begin(i),fnor_begin(i));
    }
...
double Mesh::point2TriSquareDistance(const double *P,uint find,double *cp){
    ...
    return Utility::point2TriSquareDistance(P,v1,v2,v3,cp);
...
//    }
        vector<uint>ringneighbor;vector< unsigned long>ringv2v_acc;
        BuildRingNeighborhood(1,vertices2vertices,vertices2edges_accumulate,
                              ringneighbor,ringv2v_acc);
...
void SurfaceFFFairing_OneIter(vector<double>& points, vector<double>&pointnor,
                              ...
                              double coef,vector<bool>*staticv = NULL){
    //auto oldpoints = points;
    ...
    for(int i=0;i<numV;++i){
        ...
        if(!point_nonmanifold[i]){
            ...
            weightedAddVec(-0.3,p_sec+ind,p_cv);
        }
    }
...
void Mesh::SurfaceFFFairing(double lambda,int iter,vector<bool>* staticv){
    if (!isbuildneighbortable)BuildNeighborTable();
